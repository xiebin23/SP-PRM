...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset_name", type=str, default="weepcat/hh-rlhf", required=True)
    ...
    if args.partial_process_method == "token_by_token":
        train_ds.push_to_hub(f"weepcat/hh_partial_reward_model_{args.partial_process_method}", split="train")
        test_ds.push_to_hub(f"weepcat/hh_partial_reward_model_{args.partial_process_method}", split="test")
    else:
        train_ds = concatenate_datasets([train_ds, raw_train_ds], axis=0)
        test_ds = concatenate_datasets([test_ds, raw_test_ds], axis=0)
        train_ds.push_to_hub(f"weepcat/hh_partial_reward_model_{args.partial_process_method}-{args.tag}", split="train")
        test_ds.push_to_hub(f"weepcat/hh_partial_reward_model_{args.partial_process_method}-{args.tag}", split="test")
